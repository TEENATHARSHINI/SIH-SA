...
async def test_api_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"    ✅ Summary length: {len(result.get('summary_text', ''))} chars")
            print(f"    ✅ Key sentences: {len(result.get('key_sentences', []))}")
        else:
        ...
        print("  ⚠️ httpx not available for API testing, skipping endpoint tests")
        return True
    except Exception as e:
...
def check_dependencies():
    ...
    if missing:
        print("
⚠️ Missing dependencies (optional for basic functionality):"        for package, description in missing.items():
            print(f"    - {package}: {description}")
...
async def main():
    ...
    print("\n🌐 Note: Make sure the API server is running for endpoint tests")
    ...
    if passed_tests == total_tests:
        ...
        if missing_deps:
            print("
⚠️ Optional enhancements available:"            print("  Install additional dependencies for advanced features:")
